...
::: tip 忘了填密码？
面板会显示"还没有设置管理员密码"。在 Cloudflare 控制台打开这个 Worker 的 **设置 → 变量和机密**，添加**机密（Secret）** `ADMIN_PASSWORD`，刷新页面即可。
:::

::: warning 要加「机密」，不要加「变量」
同一个页面里既能加**机密**也能加**变量**，但只有机密能活过重新部署：部署配置里没有声明任何变量，而 `wrangler deploy` 只保留机密、会清掉明文变量。如果密码是以变量形式添加的，**每次重建之后都会重新提示"还没有设置管理员密码"**。

密码只需要设置这一次：面板第一次读到机密时会把它的加盐哈希存进自己的 D1，之后即使机密丢了也照样能登录。
:::
...
3. 部署后在这个 Worker 的 **设置 → 变量和机密** 添加 `ADMIN_PASSWORD`，类型要选**机密（Secret）**而不是变量——明文变量会在每次部署时被清掉。
...
| 名称 | 类型 | 作用 |
| --- | --- | --- |
| `ADMIN_PASSWORD` | 机密（必填） | 后台登录密码，至少 8 位；修改后所有已登录的会话失效。只需设置一次：面板第一次读到后会把加盐哈希存进 D1，重新部署丢了机密也仍能登录。**必须用机密，用变量会在每次部署时被清掉** |
